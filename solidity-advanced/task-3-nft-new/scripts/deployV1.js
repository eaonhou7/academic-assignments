...
async function main() {
  ...
  const eaonNftProxy = await upgrades.deployProxy(
    EaonNft,                  // 逻辑合约工厂
    ["EAON NFT", "EAO", "dfasflasjflas"], // initialize 函数参数
    {
  ...
  // 保存合约地址到文件
  const proxyNftV1Path = path.resolve(__dirname, "./.cache/proxyNftV1.json");
  fs.writeFileSync(
      proxyNftV1Path,
      JSON.stringify({
          eaonNftProxyAddress,
  ...
  const auctionProxyV1Path = path.resolve(__dirname, "./.cache/auctionProxyV1.json");
  fs.writeFileSync(
      auctionProxyV1Path,
      JSON.stringify({
